...
TARGET_URL = "https://www.cnipa.gov.cn/col/col75/index.html"
...
def scrape_data():
    ...
    try:
        ...
        container = soup.find('div', id='17035')
        if not container:
            print('[ERROR] 国家知识产权局爬虫：未找到目标容器')
            return policies, all_items

        script_tag = container.find('script', type='text/xml')
        if not script_tag:
            print('[ERROR] 国家知识产权局爬虫：未找到XML数据')
            return policies, all_items

        xml_content = script_tag.string
        if not xml_content:
            print('[ERROR] 国家知识产权局爬虫：XML内容为空')
            return policies, all_items

        try:
            root = ET.fromstring(xml_content)
        except ET.ParseError:
            print('[ERROR] 国家知识产权局爬虫：XML解析失败')
            return policies, all_items

        records = root.findall('.//record')
        if not records:
            print('[ERROR] 国家知识产权局爬虫：未找到记录')
            return policies, all_items
            ...
            try:
                ...
                if not article_url.startswith('http'):
                    if article_url.startswith('/'):
                        article_url = "https://www.cnipa.gov.cn" + href
                    else:
                        article_url = "https://www.cnipa.gov.cn/" + href
        ...
        print(f'[OK] 国家知识产权局爬虫：成功抓取 {len(policies)} 条前一天数据')
        ...
        print(f'[ERROR] 国家知识产权局爬虫：抓取失败 - {e}')
        import traceback
        traceback.print_exc()
        print("----------------------------------------")
...
def save_to_supabase(data_list):
    try:
        ...
        return save_to_policy(data_list, "国家知识产权局")
    except Exception:
        return data_list
...
def run():
    try:
        ...
        print(f'[ERROR] 爬虫 国家知识产权局 运行失败 - {e}')
        print("----------------------------------------")
        return []
