...
TARGET_URL = "https://zwgk.mct.gov.cn/zfxxgkml/503/510/index_3081.html"
...
def scrape_data():
    ...
    try:
        ...
        container = soup.find('div', class_=lambda x: x and 'rightBox' in x)
        if not container:
            print('[ERROR] 文化和旅游部政府信息公开爬虫：未找到目标容器 div.rightBox')
            return policies, all_items

        ul_element = container.find('ul')
        if not ul_element:
            print('[ERROR] 文化和旅游部政府信息公开爬虫：未找到列表 ul')
            return policies, all_items

        li_elements = ul_element.find_all('li')
        if not li_elements:
            print('[ERROR] 文化和旅游部政府信息公开爬虫：列表为空')
            return policies, all_items
            ...
            try:
                ...
                if not article_url.startswith('http'):
                    if article_url.startswith('/'):
                        ...
                        article_url = "https://zwgk.mct.gov.cn/zfxxgkml/503/510/" + href[2:]
                    elif article_url.startswith('../'):
                        article_url = "https://zwgk.mct.gov.cn/zfxxgkml/503/" + href[3:]
                    else:
                        article_url = "https://zwgk.mct.gov.cn/zfxxgkml/503/510/" + href
        ...
        print(f'[OK] 文化和旅游部政府信息公开爬虫：成功抓取 {len(policies)} 条前一天数据')
        ...
        print(f'[ERROR] 文化和旅游部政府信息公开爬虫：抓取失败 - {e}')
        print("----------------------------------------")
...
def save_to_supabase(data_list):
    try:
        ...
        return save_to_policy(data_list, "文化和旅游部_政府信息公开")
    except Exception:
        return data_list
...
def run():
    try:
        ...
        print(f'[ERROR] 爬虫 文化和旅游部政府信息公开 运行失败 - {e}')
        print("----------------------------------------")
        return []
