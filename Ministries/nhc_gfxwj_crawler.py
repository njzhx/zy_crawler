...
TARGET_URL = "https://www.nhc.gov.cn/wjw/gfxwjj/list.shtml"
...
def scrape_data():
    ...
    try:
        ...
        ul_element = soup.find('ul', class_='zxxx_list')
        if not ul_element:
            ul_element = soup.find('ul')
        
        if not ul_element:
            print('[ERROR] 国家卫生健康委员会规范性文件爬虫：未找到列表 ul')
            ...
            try:
                ...
                if not article_url.startswith('http'):
                    if article_url.startswith('/'):
                        article_url = "https://www.nhc.gov.cn" + href
                    elif article_url.startswith('./'):
                        article_url = "https://www.nhc.gov.cn/wjw/gfxwjj/" + href[2:]
                    else:
                        article_url = "https://www.nhc.gov.cn/" + href

                pub_at = None
                span_tag = li.find('span', class_='ml')
                if span_tag:
                ...
                try:
                    ...
                    if not content_elem:
                        content_elem = detail_soup.find('div', class_=lambda x: x and ('main' in x or 'body' in x or 'text' in x))
                    if not content_elem:
                        content_elem = detail_soup.find('div')
        ...
        print(f'[ERROR] 国家卫生健康委员会规范性文件爬虫：抓取失败 - {e}')
        print("----------------------------------------")
