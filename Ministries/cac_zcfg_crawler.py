...
TARGET_URL = "https://www.cac.gov.cn/wxzw/zcfg/gfxwj/A09370305index_1.htm"
...
def scrape_data():
    ...
    try:
        ...
        if not side_column:
            print('[ERROR] 国家互联网信息办公室爬虫：未找到目标容器 div.side-right-column')
            return policies, all_items

        loading_info = side_column.find('div', id='loadingInfoPage')
        if not loading_info:
            print('[ERROR] 国家互联网信息办公室爬虫：未找到容器 div#loadingInfoPage')
            return policies, all_items

        li_elements = loading_info.find_all('li')
        if not li_elements:
            print('[ERROR] 国家互联网信息办公室爬虫：列表为空')
            return policies, all_items
        ...
        for li in li_elements:
            try:
                ...
                try:
                    ...
                    if not content_elem:
                        content_elem = detail_soup.find('div', id='detail')
                    if not content_elem:
        ...
        print(f'[OK] 国家互联网信息办公室爬虫：成功抓取 {len(policies)} 条前一天数据')
        ...
        print(f'[ERROR] 国家互联网信息办公室爬虫：抓取失败 - {e}')
        import traceback
        traceback.print_exc()
        print("----------------------------------------")
...
def run():
    try:
        ...
        print(f'[ERROR] 爬虫 国家互联网信息办公室 运行失败 - {e}')
        print("----------------------------------------")
        return []
